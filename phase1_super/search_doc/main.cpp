...
void read_superinfo()
{
    ...
    while(getline(fin, line))
    {
        ...
        double tf;
        char c; // ":"
        unordered_map<int, double> *m = new unordered_map<int, double>;
        m->clear();
        while((iss >> did >> c >> tf))
        {
            //cout << did << " " << tf << endl;
...
int main(int argc, char** argv)
{
    ...
    scoring();
    // added by Susen
    cout << "Duration without I/O: " << duration << endl;
    // added by Susen
    return 0;
}
